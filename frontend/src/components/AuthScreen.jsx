...
const AuthScreen = ({ onSubmit }) => {
  ...
  const handleSubmit = () => {
    const trimEmail = email.trim() || "user@example.com";
    const trimName = name.trim() || trimEmail.split("@")[0].replace(/[._]/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
    onSubmit({ name: trimName, email: trimEmail });
  };
      ...
      <main className="auth-form-wrap">
        <div className="auth-form">
          <div style={{ marginBottom: 4 }}>
            <div className="auth-welcome-title" style={{ fontSize: 28, fontWeight: 800, color: "#0F172A", letterSpacing: "-0.01em" }}>Welcome Back</div>
          ...
          <button className="auth-google" onClick={handleSubmit}>
          ...
          <button className="lb-btn lb-btn--primary auth-submit" onClick={handleSubmit}>
